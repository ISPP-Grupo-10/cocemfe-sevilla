    ...
    if request.user.is_superuser:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                create_event(request = request, title=f'Inicio: {document.name}', description='Inicio periodo aportaciones', creator= request.user, datetime=datetime.combine(suggestion_start_date, time(23, 59, 00)), document=document, type='aportaciones')
                create_event(request = request, title=f'Final: {document.name}', description='Final periodo aportaciones', creator= request.user, datetime=datetime.combine(suggestion_end_date, time(23, 59, 00)), document=document, type='aportaciones')
                create_event(request = request, title=f'Inicio: {document.name}', description='Inicio periodo votaciones', creator= request.user, datetime=datetime.combine(suggestion_end_date, time(23, 59, 00)), document=document, type='votaciones')
                create_event(request = request, title=f'Final: {document.name}', description='Final periodo votaciones', creator= request.user, datetime=datetime.combine(voting_end_date, time(23, 59, 00)), document=document, type='votaciones')
